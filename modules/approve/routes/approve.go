...
func ApproveClients(c *fiber.Ctx) error {
	...
	if err := database.DBConn.Raw("SELECT * FROM ewallet_web.approve_client(?,?,?)", request.Id_input, request.Cid_input, request.Mobile_input).Find(&result).Error; err != nil {
